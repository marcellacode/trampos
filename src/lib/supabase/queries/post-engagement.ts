...
export async function fetchPostEngagementBatch(
  ...
  const { data: counts, error: countsError } = await supabase.from("post_counts")
  ...
  const { data: reactions, error: reactionsError } = await supabase.from("post_reactions")
    .select("post_id")
...
export async function togglePostLike(
  ...
  const { data: existing, error: existingError } = await supabase.from("post_reactions")
    .select("post_id")
    .eq("post_id", postId)
    .eq("user_id", userId)
    .maybeSingle();
...
export async function createPostComment(
  ...
  if (input.parentCommentId) {
    const { data: parent, error: parentError } = await supabase.from("post_comments")
...
export async function shareFeedPost(
  ...
  const { data: existingShare, error: existingError } = await supabase.from("post_shares")
    .select("id")
  ...
  const { data: feedPostRow, error: feedPostError } = await supabase.from("posts")
    .insert({
