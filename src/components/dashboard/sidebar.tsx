...
function NavLink({ item, pathname, onClose }: { item: NavItem; pathname: string; onClose?: () => void; }) {
  ...
  return <Link href={item.href} onClick={onClose} className={cn("group flex min-h-10 items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/60", active ? "bg-white text-[#3d1b9b] shadow-sm" : "text-white/70 hover:bg-white/10 hover:text-white")} aria-current={active ? "page" : undefined}><Icon className={cn("h-[18px] w-[18px] shrink-0 transition-colors", active ? "text-[#5a35e8]" : "text-white/60 group-hover:text-white")} aria-hidden="true" /><span className="truncate">{item.label}</span>{item.badge ? <span className="ml-auto flex h-5 min-w-5 items-center justify-center rounded-full bg-white px-1.5 text-[10px] font-bold text-[#3d1b9b]">{item.badge}</span> : null}</Link>;
}
export function Sidebar({ user, open = false, onClose, className }: SidebarProps) {
  ...
  const content = <div className="flex h-full flex-col bg-sidebar text-sidebar-foreground">
    ...
    <nav className="flex-1 overflow-y-auto px-3 py-5" aria-label="Menu principal"><div className="space-y-6">{navSections.map((section) => <div key={section.id}><p className="mb-2 px-3 text-[10px] font-bold uppercase tracking-[0.16em] text-white/45">{section.label}</p><ul className="space-y-1" role="list">{section.items.map((item) => <li key={item.href}><NavLink item={item} pathname={pathname} onClose={onClose} /></li>)}</ul></div>)}</div></nav>
    <div className="relative border-t border-border/80 p-3">{profileOpen && <div className="absolute bottom-full left-3 right-3 mb-2 overflow-hidden rounded-xl border border-border bg-popover p-1 shadow-2xl"><Link href="/dashboard/configuracoes" onClick={() => { setProfileOpen(false); onClose?.(); }} className="flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm text-foreground transition-colors hover:bg-muted"><Settings className="h-4 w-4" aria-hidden="true" />Configurações</Link><button type="button" onClick={handleSignOut} disabled={signingOut} className="flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm text-foreground transition-colors hover:bg-muted disabled:opacity-60">{signingOut ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogOut className="h-4 w-4" />}Sair</button></div>}
      <button type="button" onClick={() => setProfileOpen((v) => !v)} className="flex w-full items-center gap-3 rounded-xl px-2 py-2.5 text-left transition-colors hover:bg-white/10" aria-expanded={profileOpen}><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-xs font-bold text-[#3d1b9b] shadow-lg">{user.initials}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold text-white">{user.name}</p><p className="truncate text-xs text-white/60">{user.plan}</p></div><ChevronUp className={cn("h-4 w-4 text-white/60 transition-transform", profileOpen ? "rotate-0" : "rotate-180")} aria-hidden="true" /></button>
    </div></div>;
